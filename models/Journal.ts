...
class Journal {
  ...
  deleteLine() {
    const CopiasLineas: JournalLine[] = this.lines.slice();
    console.log(`Linea inicial`, CopiasLineas);

    //Eliminando linea
    let NuevoArray: JournalLine[];
    NuevoArray = this.lines.filter((a, index) => index != 2);
    console.log(`Linea Eliminada`, NuevoArray);
  }
}
